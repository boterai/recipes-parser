...
from src.models.merged_recipe import MergedRecipeORM, MergedRecipe, merged_recipe_images, MergedRecipePagesORM
...
class MergedRecipeRepository(BaseRepository[MergedRecipeORM]):
    ...
    def update_merged_recipe(
        ...
        try:
            ...
            if merged_recipe.page_ids:
                ...
                # Синхронизируем merged_recipe_pages
                existing_page_ids = {
                    row[0] for row in session.query(MergedRecipePagesORM.page_id)
                    .filter(MergedRecipePagesORM.merged_recipe_id == merged_recipe_id)
                    .all()
                }
                new_set = set(sorted_ids)

                # Удаляем страницы которых больше нет
                to_delete = existing_page_ids - new_set
                if to_delete:
                    session.query(MergedRecipePagesORM).filter(
                        MergedRecipePagesORM.merged_recipe_id == merged_recipe_id,
                        MergedRecipePagesORM.page_id.in_(to_delete)
                    ).delete(synchronize_session=False)

                # Добавляем новые страницы
                to_add = new_set - existing_page_ids
                if to_add:
                    session.execute(
                        insert(MergedRecipePagesORM.__table__).values([
                            {'merged_recipe_id': merged_recipe_id, 'page_id': pid}
                            for pid in to_add
                        ])
                    )
    ...
    def create_merged_recipe(
        ...
        try:
            ...
            # Заполняем merged_recipe_pages
            page_links = [
                {'merged_recipe_id': merged_recipe_orm.id, 'page_id': pid}
                for pid in page_ids
            ]
            session.execute(insert(MergedRecipePagesORM.__table__).values(page_links))
            session.commit()
    ...
    def create_merged_recipes_batch(
        ...
        try:
            ...
            for pages_hash, merged_recipe in recipe_by_hash.items():
                ...
                # Сохраняем image_ids и page_ids для последующей связки
                if merged_recipe.image_ids:
                    recipe_image_mapping[merged_recipe_orm] = merged_recipe.image_ids
                recipe_pages_mapping[merged_recipe_orm] = sorted_ids
            ...
            if new_recipes:
                ...
                # Обновляем ID для возврата и собираем связи для промежуточных таблиц
                image_links = []
                page_links = []
                for orm_obj in new_recipes:
                    ...
                    # Собираем связи page_id → merged_recipe_id
                    for pid in recipe_pages_mapping.get(orm_obj, []):
                        page_links.append({
                            'merged_recipe_id': orm_obj.id,
                            'page_id': pid
                        })
                ...
                # Batch insert связей в merged_recipe_pages
                if page_links:
                    session.execute(insert(MergedRecipePagesORM.__table__).values(page_links))
                    logger.info(f"✓ Добавлено {len(page_links)} записей в merged_recipe_pages")
                
                if image_links or page_links:
                    session.commit()
    ...
    def get_recipes_for_cluster(
        self,
        page_ids: list[int],
        skip_variations: bool = True
    ) -> list[MergedRecipeORM]:
        """
        Получить рецепты для страниц кластера.

        Args:
            page_ids: Список page_id страниц кластера
            min_recipe_count: Минимальное количество страниц в рецепте (по умолчанию 1 — хотим больше)
        ...
        try:
            query = (
                ...
                .join(
                    MergedRecipePagesORM,
                    (MergedRecipePagesORM.merged_recipe_id == MergedRecipeORM.id) &
                    (MergedRecipePagesORM.page_id.in_(page_ids))
                ))
            ...
            logger.error(f"Ошибка получения незавершённых рецептов: {e}")
            raise
        finally:
            session.close()
...
class MergedRecipePagesRepository(BaseRepository[MergedRecipePagesORM]):
    """Репозиторий для работы с промежуточной таблицей merged_recipe_pages"""
    
    def __init__(self, mysql_manager=None):
        if mysql_manager is None:
            mysql_manager = get_db_connection()
        super().__init__(MergedRecipePagesORM, mysql_manager)