...
Is this a recipe page? If yes, is the extraction correct? If data is missing, extract it from HTML. Return JSON validation results."""
    ...
    def validate(self, module_name: str, use_gpt: bool = False, required_fields: list[str] = None, use_gpt_on_errors_only: bool = True):
        """
        ...
        logger.info(f"Валидация завершена: {passed}/{total} файлов прошли проверку")
...
if __name__ == "__main__":
    ...
    """
    порядок использвоания 
    сборка всех веток 
    переключение на каждую и запуск валидации с нужными параметрами
    git log main..copilot/add-recipe-parser-delamaris --diff-filter=A --name-only --pretty=format: | sort -u

    сохранение результатов в файл
    мердж при успехе и отсутсвтие такогового при ошибке
