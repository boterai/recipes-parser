## Задача: реализовать парсер рецептов для сайта spendsmart.extension.iastate.edu
---

Нужно реализовать новый парсер для сайта **spendsmart.extension.iastate.edu**.  
...
В репозитории уже сохранены примеры (директория `preprocessed/spendsmart_extension_iastate_edu`):

- HTML‑файлы страниц рецептов сайта **spendsmart.extension.iastate.edu**
...
- Создать новый модуль:
  ...
  - `extractor/spendsmart_extension_iastate_edu.py`

- В этом модуле реализовать класс парсера для **spendsmart.extension.iastate.edu**.
...
- Извлечение данных должно опираться на реальную HTML‑структуру **spendsmart.extension.iastate.edu**:
  - теги,
  - классы,
  - атрибуты,
...
- `image_urls`:  
  ...
  Пример:
  ```json
  "image_urls": "https://spendsmart.extension.iastate.edu/images/recipe1.jpg,https://spendsmart.extension.iastate.edu/images/recipe2.jpg"
...
1. Для каждого HTML‑примера из `preprocessed/spendsmart_extension_iastate_edu`:
...
- Работать следует в отдельной ветке, например:
  - `feature/spendsmart_extension_iastate_edu-parser`
- Целевая ветка для Pull Request:
  - `feature/copilot-issue`.
- В PR указать:
  - ссылку на этот issue;
  - краткое описание реализованного парсера;
  - список файлов, затронутых изменениями (в первую очередь `extractor/spendsmart_extension_iastate_edu.py`);

---

### Критерии готовности задачи

1. В файле `extractor/spendsmart_extension_iastate_edu.py` создан класс парсера для **spendsmart.extension.iastate.edu**, наследующий базовый класс (как парсер allrecipes).
2. Метод(ы) парсера:
   - принимают HTML‑страницу рецепта spendsmart.extension.iastate.edu (путь к файлу, уже реализовано в базовом классе, нужно реализовать только остальные методы),
