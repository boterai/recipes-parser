## Задача: реализовать парсер рецептов для сайта evelynscooking.com
---

Нужно реализовать новый парсер для сайта **evelynscooking.com**.  
...
В репозитории уже сохранены примеры (директория `preprocessed/evelynscooking_com`):

- HTML‑файлы страниц рецептов сайта **evelynscooking.com**
...
- Создать новый модуль:
  ...
  - `extractor/evelynscooking_com.py`

- В этом модуле реализовать класс парсера для **evelynscooking.com**.
...
- Извлечение данных должно опираться на реальную HTML‑структуру **evelynscooking.com**:
  - теги,
  - классы,
  - атрибуты,
...
- `image_urls`:  
  ...
  Пример:
  ```json
  "image_urls": "https://evelynscooking.com/images/recipe1.jpg,https://evelynscooking.com/images/recipe2.jpg"
...
1. Для каждого HTML‑примера из `preprocessed/evelynscooking_com`:
...
- Работать следует в отдельной ветке, например:
  - `feature/evelynscooking_com-parser`
- Целевая ветка для Pull Request:
  - `feature/copilot-issue`.
- В PR указать:
  - ссылку на этот issue;
  - краткое описание реализованного парсера;
  - список файлов, затронутых изменениями (в первую очередь `extractor/evelynscooking_com.py`);

---

### Критерии готовности задачи

1. В файле `extractor/evelynscooking_com.py` создан класс парсера для **evelynscooking.com**, наследующий базовый класс (как парсер allrecipes).
2. Метод(ы) парсера:
   - принимают HTML‑страницу рецепта evelynscooking.com (путь к файлу, уже реализовано в базовом классе, нужно реализовать только остальные методы),
