## Задача: реализовать парсер рецептов для сайта sallysbakingaddiction.com
---

Нужно реализовать новый парсер для сайта **sallysbakingaddiction.com**.  
...
В репозитории уже сохранены примеры (директория `preprocessed/sallysbakingaddiction_com`):

- HTML‑файлы страниц рецептов сайта **sallysbakingaddiction.com**
...
- Создать новый модуль:
  ...
  - `extractor/sallysbakingaddiction_com.py`

- В этом модуле реализовать класс парсера для **sallysbakingaddiction.com**.
...
- Извлечение данных должно опираться на реальную HTML‑структуру **sallysbakingaddiction.com**:
  - теги,
  - классы,
  - атрибуты,
...
- `image_urls`:  
  ...
  Пример:
  ```json
  "image_urls": "https://sallysbakingaddiction.com/images/recipe1.jpg,https://sallysbakingaddiction.com/images/recipe2.jpg"
...
1. Для каждого HTML‑примера из `preprocessed/sallysbakingaddiction_com`:
...
- Работать следует в отдельной ветке, например:
  - `feature/sallysbakingaddiction_com-parser`
- Целевая ветка для Pull Request:
  - `feature/copilot-issue`.
- В PR указать:
  - ссылку на этот issue;
  - краткое описание реализованного парсера;
  - список файлов, затронутых изменениями (в первую очередь `extractor/sallysbakingaddiction_com.py`);

---

### Критерии готовности задачи

1. В файле `extractor/sallysbakingaddiction_com.py` создан класс парсера для **sallysbakingaddiction.com**, наследующий базовый класс (как парсер allrecipes).
2. Метод(ы) парсера:
   - принимают HTML‑страницу рецепта sallysbakingaddiction.com (путь к файлу, уже реализовано в базовом классе, нужно реализовать только остальные методы),
