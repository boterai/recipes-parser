## Задача: реализовать парсер рецептов для сайта ritzyrecipes.com
---

Нужно реализовать новый парсер для сайта **ritzyrecipes.com**.  
...
В репозитории уже сохранены примеры (директория `preprocessed/ritzyrecipes_com`):

- HTML‑файлы страниц рецептов сайта **ritzyrecipes.com**
...
- Создать новый модуль:
  ...
  - `extractor/ritzyrecipes_com.py`

- В этом модуле реализовать класс парсера для **ritzyrecipes.com**.
...
- Извлечение данных должно опираться на реальную HTML‑структуру **ritzyrecipes.com**:
  - теги,
  - классы,
  - атрибуты,
...
- `image_urls`:  
  ...
  Пример:
  ```json
  "image_urls": "https://ritzyrecipes.com/images/recipe1.jpg,https://ritzyrecipes.com/images/recipe2.jpg"
...
1. Для каждого HTML‑примера из `preprocessed/ritzyrecipes_com`:
...
- Работать следует в отдельной ветке, например:
  - `feature/ritzyrecipes_com-parser`
- Целевая ветка для Pull Request:
  - `feature/copilot-issue`.
- В PR указать:
  - ссылку на этот issue;
  - краткое описание реализованного парсера;
  - список файлов, затронутых изменениями (в первую очередь `extractor/ritzyrecipes_com.py`);

---

### Критерии готовности задачи

1. В файле `extractor/ritzyrecipes_com.py` создан класс парсера для **ritzyrecipes.com**, наследующий базовый класс (как парсер allrecipes).
2. Метод(ы) парсера:
   - принимают HTML‑страницу рецепта ritzyrecipes.com (путь к файлу, уже реализовано в базовом классе, нужно реализовать только остальные методы),
