## Задача: реализовать парсер рецептов для сайта {SITE_DOMAIN}
---

Нужно реализовать новый парсер для сайта **{SITE_DOMAIN}**.  
...
В репозитории уже сохранены примеры (директория `preprocessed/{MODULE_NAME}`):

- HTML‑файлы страниц рецептов сайта **{SITE_DOMAIN}**
...
- В этом модуле реализовать класс парсера для **{SITE_DOMAIN}**.
...
- Извлечение данных должно опираться на реальную HTML‑структуру **{SITE_DOMAIN}**:
  - теги,
  - классы,
  - атрибуты,
...
- `image_urls`:  
  Список URL изображений рецепта (фотографии готового блюда, процесса приготовления и т.д.).  
  Формат: строка, в которой url с изображениями перечислены через запятую без пробелов
  Пример:
  ```json
...
1. Для каждого HTML‑примера из `preprocessed/{MODULE_NAME}`:
...
- Работать следует в отдельной ветке, например:
  - `feature/{MODULE_NAME}-parser`
- Целевая ветка для Pull Request:
  - `feature/copilot-issue`.
- В PR указать:
  - ссылку на этот issue;
  - краткое описание реализованного парсера;
  - список файлов, затронутых изменениями (в первую очередь `extractor/{MODULE_NAME}.py`);

---

### Критерии готовности задачи

1. В файле `extractor/{MODULE_NAME}.py` создан класс парсера для **{SITE_DOMAIN}**, наследующий базовый класс (как парсер allrecipes).
2. Метод(ы) парсера:
   - принимают HTML‑страницу рецепта {SITE_DOMAIN} (путь к файлу, уже реализовано в базовом классе, нужно реализовать только остальные методы),
