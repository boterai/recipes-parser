"""
Тесты для экстрактора receptfavoriter.se
"""
...
from extractor.receptfavoriter_se import ReceptfavoriterSeExtractor


class TestReceptfavoriterSeExtractor(unittest.TestCase):
    """Тесты для ReceptfavoriterSeExtractor"""
    
    @classmethod
    def setUpClass(cls):
        """Подготовка - находим тестовые HTML файлы"""
        cls.test_dir = Path(__file__).parent.parent.parent / "preprocessed" / "receptfavoriter_se"
        cls.html_files = list(cls.test_dir.glob("*.html"))
        
        if not cls.html_files:
            raise unittest.SkipTest("No HTML test files found in preprocessed/receptfavoriter_se")
    ...
    def test_extractor_initialization(self):
        ...
        extractor = ReceptfavoriterSeExtractor(str(html_file))
    ...
    def test_extract_times(self):
        ...
        # Проверяем что хотя бы одно время извлечено
        self.assertTrue(
    ...
    def test_extract_image_urls(self):
        ...
        if image_urls:
            ...
            self.assertTrue('http' in image_urls)
