"""
Тесты для экстрактора clickpoftabuna.ro
"""
...
from extractor.clickpoftabuna_ro import ClickPoftabunaExtractor


class TestClickPoftabunaExtractor(unittest.TestCase):
    """Тесты для ClickPoftabunaExtractor"""
    
    @classmethod
    def setUpClass(cls):
        """Подготовка - находим тестовые HTML файлы"""
        cls.test_dir = Path(__file__).parent.parent.parent / "preprocessed" / "clickpoftabuna_ro"
        cls.html_files = list(cls.test_dir.glob("*.html"))
        
        if not cls.html_files:
            raise unittest.SkipTest("No HTML test files found in preprocessed/clickpoftabuna_ro")
    ...
    def test_extractor_initialization(self):
        ...
        extractor = ClickPoftabunaExtractor(str(html_file))
    ...
    def test_parse_ingredient_with_amount_and_unit(self):
        ...
        extractor = ClickPoftabunaExtractor(str(self.html_files[0]))
        
        result = extractor.parse_ingredient("250 g de ravioli al funghi")
        
        self.assertIsNotNone(result)
        self.assertIn('ravioli', result['name'].lower())
        self.assertEqual(result['amount'], 250)
        self.assertEqual(result['units'], 'g')
    
    def test_parse_ingredient_with_linguri(self):
        """Тест парсинга ингредиента с единицей 'linguri'"""
        extractor = ClickPoftabunaExtractor(str(self.html_files[0]))
        
        result = extractor.parse_ingredient("2 linguri muștar Dijon")
        
        self.assertIsNotNone(result)
        self.assertEqual(result['name'], 'muștar Dijon')
        self.assertEqual(result['amount'], 2)
        self.assertEqual(result['units'], 'linguri')
    
    def test_parse_ingredient_with_lingura(self):
        """Тест парсинга ингредиента с единицей 'lingură'"""
        extractor = ClickPoftabunaExtractor(str(self.html_files[0]))
        
        result = extractor.parse_ingredient("1 lingură maioneză")
        
        self.assertIsNotNone(result)
        self.assertEqual(result['name'], 'maioneză')
        self.assertEqual(result['amount'], 1)
        self.assertEqual(result['units'], 'lingură')
    
    def test_parse_ingredient_without_unit(self):
        """Тест парсинга ингредиента без единицы измерения"""
        extractor = ClickPoftabunaExtractor(str(self.html_files[0]))
        
        result = extractor.parse_ingredient("6 cartofi")
        
        self.assertIsNotNone(result)
        self.assertEqual(result['name'], 'cartofi')
    ...
    def test_parse_ingredient_without_amount(self):
        ...
        result = extractor.parse_ingredient("sare")
        
        self.assertIsNotNone(result)
        self.assertEqual(result['name'], 'sare')
    ...
    def test_extract_instructions_not_empty(self):
        ...
        # Instructions должны быть либо None, либо непустой строкой
    ...
    def test_extract_image_urls(self):
        ...
        if image_urls:
            ...
            self.assertTrue(
                'http://' in image_urls or 'https://' in image_urls,
                "Image URL should contain http:// or https://"
            )
...
class TestClickPoftabunaExtractorEdgeCases(unittest.TestCase):
