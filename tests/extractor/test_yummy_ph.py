"""
Тесты для экстрактора yummy.ph
"""
...
from extractor.yummy_ph import YummyPhExtractor


class TestYummyPhExtractor(unittest.TestCase):
    """Тесты для YummyPhExtractor"""
    
    @classmethod
    def setUpClass(cls):
        """Подготовка - находим тестовые HTML файлы"""
        cls.test_dir = Path(__file__).parent.parent.parent / "preprocessed" / "yummy_ph"
        cls.html_files = list(cls.test_dir.glob("*.html"))
        
        if not cls.html_files:
            raise unittest.SkipTest("No HTML test files found in preprocessed/yummy_ph")
    ...
    def test_extractor_initialization(self):
        ...
        extractor = YummyPhExtractor(str(html_file))
    ...
    def test_parse_iso_duration(self):
        ...
        # PT15M -> 15 minutes
        result = YummyPhExtractor.parse_iso_duration("PT15M")
        self.assertEqual(result, "15 minutes")
        
        # PT1H -> 1 hour
        result = YummyPhExtractor.parse_iso_duration("PT1H")
        self.assertEqual(result, "1 hour")
        
        # PT1H30M -> 1 hour 30 minutes
        result = YummyPhExtractor.parse_iso_duration("PT1H30M")
        self.assertEqual(result, "1 hour 30 minutes")
        
        # PT2H -> 2 hours
        result = YummyPhExtractor.parse_iso_duration("PT2H")
