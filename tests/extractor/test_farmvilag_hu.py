"""
Тесты для экстрактора farmvilag.hu
"""
...
class TestFarmvilagExtractor(unittest.TestCase):
    """Тесты для FarmvilagExtractor"""
    
    @classmethod
    def setUpClass(cls):
        """Подготовка - находим тестовые HTML файлы"""
        cls.test_dir = Path(__file__).parent.parent.parent / "preprocessed" / "farmvilag_hu"
        cls.html_files = list(cls.test_dir.glob("*.html"))
        
        if not cls.html_files:
            raise unittest.SkipTest("No HTML test files found in preprocessed/farmvilag_hu")
    ...
    def test_extractor_initialization(self):
        ...
        extractor = FarmvilagExtractor(str(html_file))
    ...
    def test_parse_ingredient_with_amount_and_unit(self):
        ...
        extractor = FarmvilagExtractor(str(self.html_files[0]))
        
        result = extractor.parse_ingredient("500 g sima búzaliszt")
        
        self.assertIsNotNone(result)
        self.assertEqual(result['name'], 'sima búzaliszt')
    ...
    def test_parse_ingredient_with_range(self):
        ...
        result = extractor.parse_ingredient("350-400 ml vajtej")
        
        self.assertIsNotNone(result)
        self.assertEqual(result['name'], 'vajtej')
        self.assertEqual(result['amount'], '350')  # Берем первое значение из диапазона
        self.assertEqual(result['units'], 'ml')
    
    def test_parse_ingredient_without_amount(self):
        """Тест парсинга ингредиента без количества"""
        extractor = FarmvilagExtractor(str(self.html_files[0]))
        
        result = extractor.parse_ingredient("extra liszt")
        
        self.assertIsNotNone(result)
        self.assertEqual(result['name'], 'extra liszt')
    ...
    def test_ingredients_format_is_json_string(self):
        ...
        # Найдем файл с ингредиентами
        test_file = None
        for html_file in self.html_files:
            if 'irish' in str(html_file).lower():
                test_file = html_file
                break
        ...
        extractor = FarmvilagExtractor(str(test_file))
...
class TestFarmvilagExtractorEdgeCases(unittest.TestCase):
