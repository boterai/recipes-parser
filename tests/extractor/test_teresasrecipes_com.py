"""
Тесты для экстрактора teresasrecipes.com
"""
...
from extractor.teresasrecipes_com import TeresasRecipesExtractor


class TestTeresasRecipesExtractor(unittest.TestCase):
    """Тесты для TeresasRecipesExtractor"""
    
    @classmethod
    def setUpClass(cls):
        """Подготовка - находим тестовые HTML файлы"""
        cls.test_dir = Path(__file__).parent.parent.parent / "preprocessed" / "teresasrecipes_com"
        cls.html_files = list(cls.test_dir.glob("*.html"))
        
        if not cls.html_files:
            raise unittest.SkipTest("No HTML test files found in preprocessed/teresasrecipes_com")
    ...
    def test_extractor_initialization(self):
        ...
        extractor = TeresasRecipesExtractor(str(html_file))
    ...
    def test_extract_notes(self):
        """Тест что notes извлекаются из секции Tips"""
        if not self.html_files:
            self.skipTest("No HTML files available")
    ...
    def test_parse_iso_duration(self):
        ...
        extractor_class = TeresasRecipesExtractor
