...
class XrysessyntagesExtractor(BaseRecipeExtractor):
    ...
    def extract_dish_name(self) -> Optional[str]:
        ...
        if og_title and og_title.get('content'):
            ...
            title = re.sub(r'\s*-\s*Απλές και νόστιμες συνταγές.*$', '', title)
        ...
        for p in all_paragraphs:
            ...
            if text and 'ΥΛΙΚΑ' not in text and 'ΕΚΤΕΛΕΣΗ' not in text:
                if len(text) > 30 and not re.match(r'^\d', text):
                    cleaned = self.clean_text(text)
                    if cleaned and not any(word in cleaned.lower() for word in ['κουταλιά', 'γρ.', 'κουταλάκι', 'φλ.']):
                        return cleaned
    ...
    def extract_ingredients(self) -> Optional[str]:
        ...
        # Сначала пробуем извлечь из meta description (если ингредиенты там)
        meta_desc = self.soup.find('meta', {'name': 'description'})
        if meta_desc and meta_desc.get('content'):
            desc = meta_desc['content']
            # Проверяем, есть ли там Υλικά в текстовом виде или в HTML entities
            if 'Υλικά' in desc or 'Υλικά'.lower() in desc.lower():
                # Заменяем HTML entities
                import html
                desc = html.unescape(desc)
                # Теперь разбиваем по <br>
                ...
                for line in lines:
                    ...
                    # Начинаем сбор ингредиентов после "Υλικά"
                    if 'Υλικά' in line or 'ΥΛΙΚΑ' in line:
                        in_ingredients = True
                        continue
                    
                    # Заканчиваем при встрече "Εκτέλεση" или других разделов
                    if 'Εκτέλεση' in line or 'ΕΚΤΕΛΕΣΗ' in line or 'Εκτέλεση' in line:
                        break
                    
                    if in_ingredients:
                        # Пропускаем подзаголовки типа "Για τη γέμιση:", "Για τη σάλτσα:"
                        if line.startswith('Για') and line.endswith(':'):
                            continue
        ...
        # Если не нашли в meta, ищем параграф с "ΥΛΙΚΑ" во всех параграфах
        ...
        for p in paragraphs:
            ...
            if 'ΥΛΙΚΑ' in p.get_text():
                ...
                for line in lines:
                    line = line.strip()
                    if line and 'ΥΛΙΚΑ' not in line and not (line.startswith('Για') and line.endswith(':')):
                        # Парсим ингредиент
                        parsed = self.parse_ingredient_line(line)
                        if parsed:
                            ingredients.append(parsed)
