...
class GirlCooksWorldExtractor(BaseRecipeExtractor):
    ...
    def extract_ingredients(self) -> Optional[str]:
        ...
        # Вариант 1: Ищем div.recipe (старый формат)
        recipe_div = self.soup.find('div', class_='recipe')
        if recipe_div:
        ...
        # Вариант 2: Fallback - ищем заголовок "Ingredients:" (новый формат)
        for heading in self.soup.find_all(['h2', 'h3', 'h4']):
            heading_text = self.clean_text(heading.get_text())
            if re.search(r'\bingredients?\b', heading_text, re.IGNORECASE):
                # Нашли заголовок с ингредиентами, собираем следующие элементы
                current = heading.find_next_sibling()
                
                while current:
                    # Если встретили новый заголовок (h2-h4), останавливаемся
                    if current.name in ['h2', 'h3', 'h4']:
                        break
                    
                    # Собираем из UL/OL списков
                    ...
                    # Пропускаем подзаголовки типа "Crust:", "Filling:"
                    # но продолжаем поиск после них
    ...
    def extract_instructions(self) -> Optional[str]:
        ...
        # Вариант 2: Fallback - ищем заголовок "Instructions:" (новый формат)
        steps = []
        
        for heading in self.soup.find_all(['h2', 'h3', 'h4']):
            heading_text = self.clean_text(heading.get_text())
            if re.search(r'\binstructions?\b|\bdirections?\b|\bmethod\b', heading_text, re.IGNORECASE):
                # Нашли заголовок с инструкциями, собираем следующие элементы
                current = heading.find_next_sibling()
                ...
                while current:
                    ...
                    # Также собираем из параграфов
                    elif current.name == 'p':
                        text = self.clean_text(current.get_text())
                        # Пропускаем подзаголовки типа "The crust:"
                        if text and not (text.endswith(':') and len(text) < 30):
                            # Если это не пустой параграф и не короткий подзаголовок
                            if len(text) > 10:
                                steps.append(text)
