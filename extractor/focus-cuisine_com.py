"""
Экстрактор данных рецептов для сайта focus-cuisine.com
"""
...
class FocusCuisineExtractor(BaseRecipeExtractor):
            ...
            try:
                ...
                if isinstance(data, list):
                    ...
                    if '@graph' in data:
                        ...
                        # Проверяем непосредственно data
    ...
    def extract_dish_name(self) -> Optional[str]:
        ...
        if og_title and og_title.get('content'):
            ...
            # Убираем суффиксы типа " Recipe", " - Focus Cuisine"
            title = re.sub(r'\s+(Recipe|Focus Cuisine).*$', '', title, flags=re.IGNORECASE)
    ...
    def extract_steps(self) -> Optional[str]:
        ...
        if not steps:
            ...
            for container in instructions_containers:
                ...
                if not step_items:
                    # Может быть div с itemprop
                    step_items = container.find_all(attrs={'itemprop': 'recipeInstructions'})
    ...
    def extract_tags(self) -> Optional[str]:
        ...
        if not tags_list:
            ...
            if article_tags:
                tags_list = [tag['content'] for tag in article_tags if tag.get('content')]
    ...
    def extract_image_urls(self) -> Optional[str]:
        ...
        img_elems = self.soup.find_all('img', attrs={'itemprop': 'image'})
        for img_elem in img_elems:
            if img_elem.get('src'):
                urls.append(img_elem['src'])
            elif img_elem.get('data-src'):
                urls.append(img_elem['data-src'])
    ...
    def extract_all(self) -> dict:
        ...
        Returns:
            Словарь с данными рецепта, все поля обязательны (None если не найдено)
        """
        return {
...
def main():
    ...
    preprocessed_dir = os.path.join("preprocessed", "focus-cuisine_com")
    
    if os.path.exists(preprocessed_dir) and os.path.isdir(preprocessed_dir):
        print(f"Обработка файлов из директории: {preprocessed_dir}")
        process_directory(FocusCuisineExtractor, preprocessed_dir)
    else:
        print(f"Директория не найдена: {preprocessed_dir}")
        print("Использование: python focus-cuisine_com.py")
