...
class ReceptenpuntNlExtractor(BaseRecipeExtractor):
    ...
    def _extract_steps_from_html(self) -> Optional[str]:
        """
        Фоллбэк метод для извлечения инструкций из HTML контента
        Ищет все <ol> списки в основном контенте статьи до заголовка "Variaties"
        """
        steps = []
        
        # Ищем основной контент статьи
        article = self.soup.find('article') or self.soup.find('div', class_='entry-content')
        
        if not article:
            return None
        
        # Ключевые слова заголовков, на которых останавливаемся
        stop_headers = ['variaties', 'varianten', 'tips', 'opmerking', 'veelgestelde vragen', 'faq']
        
        # Находим все <ol> списки в статье
        step_num = 1
        
        for element in article.find_all(['ol', 'h2', 'h3']):
            # Если это заголовок, проверяем не нужно ли остановиться
            if element.name in ['h2', 'h3']:
                header_text = self.clean_text(element.get_text()).lower()
                # Останавливаемся, если встретили заголовок вариаций или советов
                if any(keyword in header_text for keyword in stop_headers):
                    break
            
            # Если это <ol> список, извлекаем шаги
            elif element.name == 'ol':
                items = element.find_all('li', recursive=False)  # Только прямые дети
    ...
    def extract_steps(self) -> Optional[str]:
        ...
        # Фоллбэк: извлечение из HTML если JSON-LD не содержит инструкций
