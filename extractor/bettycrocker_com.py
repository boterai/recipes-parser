"""
Экстрактор данных рецептов для сайта bettycrocker.com
"""
...
class BettyCrockerExtractor(BaseRecipeExtractor):
    """Экстрактор для bettycrocker.com"""
    ...
    def extract_dish_name(self) -> Optional[str]:
        ...
        if og_title and og_title.get('content'):
            ...
            # Убираем суффиксы типа " Recipe - BettyCrocker.com"
            title = re.sub(r'\s+(Recipe|BettyCrocker\.com).*$', '', title, flags=re.IGNORECASE)
    ...
    def parse_ingredient(self, ingredient_text: str) -> Optional[dict]:
        ...
        # Заменяем Unicode дроби на числа (только standalone)
        # Обрабатываем сначала Unicode дроби
        fraction_map = {
            '½': ' 0.5', '¼': ' 0.25', '¾': ' 0.75',
            '⅓': ' 0.33', '⅔': ' 0.67', '⅛': ' 0.125',
            '⅜': ' 0.375', '⅝': ' 0.625', '⅞': ' 0.875',
        ...
        # Паттерн: ловим "число пробел дробь" или просто "дробь"
        ...
        # Примеры: "1 cup flour", "2 tablespoons butter", "1.5 teaspoons salt"
        # Обновлен: учитывает диапазоны вида "4 to 6 tablespoons"
        pattern = r'^([\d.,]+(?:\s+to\s+[\d.,]+)?)?\s*(cups?|tablespoons?|teaspoons?|tbsps?|tsps?|pounds?|ounces?|lbs?\.?|oz\.?|grams?|kilograms?|g(?!\w)|kg|milliliters?|liters?|ml|l(?!\w)|pinch(?:es)?|dash(?:es)?|packages?|pkgs?|cans?|jars?|bottles?|inch(?:es)?|slices?|cloves?|bunches?|sprigs?|whole|halves?|quarters?|pieces?|heads?)?\s*(.+)'
        ...
        name = re.sub(r'\([^)]*\)', '', name)
        # Удаляем фразы "to taste", "as needed", "optional", "if desired"
        name = re.sub(r'\b(to taste|as needed|or more|if needed|optional|if desired|for garnish)\b', '', name, flags=re.IGNORECASE)
    ...
    def extract_category(self) -> Optional[str]:
        ...
        # Альтернативно - из keywords или meta tags
    ...
    def parse_iso_duration(duration: str) -> Optional[str]:
        ...
        Args:
            duration: строка вида "PT20M" или "PT1H30M" или "PT7H50M"
            
        Returns:
            Время в формате "20 minutes", "1 hour 30 minutes", "7 hours 50 minutes"
        """
    ...
    def extract_notes(self) -> Optional[str]:
        ...
        # Ищем в JSON-LD в mainEntity (FAQ секция)
        recipe_data = self._get_recipe_json_ld()
        if recipe_data and 'mainEntity' in recipe_data:
            main_entity = recipe_data['mainEntity']
            
            if isinstance(main_entity, list) and len(main_entity) > 0:
                # Берем первый вопрос-ответ как примечание
                first_qa = main_entity[0]
                if isinstance(first_qa, dict) and 'acceptedAnswer' in first_qa:
                    answer = first_qa['acceptedAnswer']
                    if isinstance(answer, dict) and 'text' in answer:
                        # Убираем HTML теги
                        note_text = re.sub(r'<[^>]+>', ' ', answer['text'])
...
def main():
    """Точка входа для обработки директории с HTML-страницами bettycrocker.com"""
    import os
    
    # Ищем директорию с HTML-страницами
    preprocessed_dir = os.path.join("preprocessed", "bettycrocker_com")
    
    if os.path.exists(preprocessed_dir) and os.path.isdir(preprocessed_dir):
        process_directory(BettyCrockerExtractor, preprocessed_dir)
        return
