"""
Экстрактор данных рецептов для сайта mealplanningblueprints.com
"""
...
class MealPlanningBlueprintsExtractor(BaseRecipeExtractor):
    """Экстрактор для mealplanningblueprints.com"""
    ...
    def extract_prep_time(self) -> Optional[str]:
        ...
        # Альтернатива - поиск в HTML
        time_elem = self.soup.find(class_=re.compile(r'prep.*time', re.I))
        if not time_elem:
            time_elem = self.soup.find(attrs={'data-test-id': re.compile(r'prep', re.I)})
    ...
    def extract_cook_time(self) -> Optional[str]:
        ...
        if not time_elem:
            time_elem = self.soup.find(attrs={'data-test-id': re.compile(r'cook', re.I)})
    ...
    def extract_total_time(self) -> Optional[str]:
        ...
        if not time_elem:
            time_elem = self.soup.find(attrs={'data-test-id': re.compile(r'total', re.I)})
    ...
    def extract_notes(self) -> Optional[str]:
        ...
        notes_section = self.soup.find(class_=re.compile(r'(note|tip|cook.*note)', re.I))
        
        if notes_section:
            # Пробуем найти параграф внутри
            ...
            # Убираем заголовки
            text = re.sub(r"^(Cook'?s?\s+)?Note[s]?\s*:?\s*", '', text, flags=re.I)
            text = re.sub(r"^Tip[s]?\s*:?\s*", '', text, flags=re.I)
            text = self.clean_text(text)
            return text if text else None
        
        # Ищем параграфы с ключевыми словами
        note_keywords = [
            'note:', 'tip:', 'you can substitute', 'feel free to',
    ...
    def extract_tags(self) -> Optional[str]:
        """Извлечение тегов из meta-тега keywords или parsely-tags"""
        tags_list = []
        ...
        for tag in tags_list:
            ...
            if tag_lower in stopwords or len(tag) < 3:
                continue
...
def main():
    ...
    preprocessed_dir = os.path.join("preprocessed", "mealplanningblueprints_com")
    
    if os.path.exists(preprocessed_dir) and os.path.isdir(preprocessed_dir):
        process_directory(MealPlanningBlueprintsExtractor, preprocessed_dir)
        return
    
    print(f"Директория не найдена: {preprocessed_dir}")
    print("Использование: python mealplanningblueprints_com.py")
