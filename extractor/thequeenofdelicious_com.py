"""
Экстрактор данных рецептов для сайта thequeenofdelicious.com
"""
...
class TheQueenOfDeliciousExtractor(BaseRecipeExtractor):
    """Экстрактор для thequeenofdelicious.com"""
    ...
    def parse_iso_duration(duration: str) -> Optional[str]:
        ...
        Args:
            duration: строка вида "PT20M" или "PT1H30M" или "PT505M"
            
        Returns:
            Время в читаемом формате, например "20 minutes" или "1 hour 30 minutes" или "8 hours 25 minutes"
        """
        ...
        # Конвертируем большие минуты в часы, если нет часов
    ...
    def _get_json_ld_article(self) -> Optional[dict]:
        """Извлечение Article данных из JSON-LD (для keywords/tags)"""
        json_ld_scripts = self.soup.find_all('script', type='application/ld+json')
        ...
        for script in json_ld_scripts:
            try:
                ...
                # Функция для проверки типа Article
                def is_article(item):
                    item_type = item.get('@type', '')
                    if isinstance(item_type, list):
                        return 'Article' in item_type
                    return item_type == 'Article'
                
                # Ищем Article в данных
                if isinstance(data, list):
                    for item in data:
                        if isinstance(item, dict) and is_article(item):
                            return item
                elif isinstance(data, dict):
                    if is_article(data):
                        return data
    ...
    def parse_ingredient(self, ingredient_text: str) -> Optional[dict]:
        ...
        # Удаляем содержимое скобок (обычно примечания)
        text_without_parens = re.sub(r'\([^)]*\)', '', text).strip()
        
        # Паттерн для извлечения количества, единицы и названия
        # Поддержка финских единиц измерения: dl, l, ml, g, kg, tl (чайная ложка), rkl (столовая ложка), kpl (штука), pala, oksa
        pattern = r'^([\d\s/.,]+)\s*(dl|l|ml|g|kg|tl|rkl|kpl|pala|oksa|cups?|tablespoons?|teaspoons?|tbsps?|tsps?|pounds?|ounces?|lbs?|oz|grams?|kilograms?|milliliters?|liters?|pinch(?:es)?|dash(?:es)?|packages?|cans?|jars?|bottles?|inch(?:es)?|slices?|cloves?|bunches?|sprigs?|whole|halves?|quarters?|pieces?|head|heads)?\s*(.+)'
        ...
        # Очистка названия
        # Удаляем звездочки в начале (маркеры примечаний)
        name = re.sub(r'^\*+\s*', '', name)
        # Удаляем фразы на финском "maun mukaan", "tarvittaessa" и т.д.
        name = re.sub(r'\b(maun mukaan|tarvittaessa|halutessasi?)\b', '', name, flags=re.IGNORECASE)
        # Обработка "или" / "tai" - берем только первый вариант
        if ' tai ' in name.lower():
            name = name.split(' tai ')[0].strip()
    ...
    def extract_ingredients(self) -> Optional[str]:
        ...
        # Ищем wprm-recipe-ingredients или другие варианты
        ingredient_containers = [
            self.soup.find('div', class_=re.compile(r'wprm-recipe-ingredients', re.I)),
    ...
    def extract_instructions(self) -> Optional[str]:
        ...
        instructions_containers = [
            self.soup.find('div', class_=re.compile(r'wprm-recipe-instructions', re.I)),
    ...
    def extract_notes(self) -> Optional[str]:
        ...
        # Ищем в HTML структуре wprm-recipe-notes
        notes_containers = [
            self.soup.find('div', class_=re.compile(r'wprm-recipe-notes', re.I)),
        ...
        for container in notes_containers:
            if container:
                ...
                # Удаляем заголовки на финском и английском (более гибкое удаление)
                text = re.sub(r"^(Notes?|Vinkit(\s+ja\s+huomiot)?|Huomioitavaa|Huomautukset)\s*:?\s*", '', text, flags=re.I)
    ...
    def extract_tags(self) -> Optional[str]:
        ...
        # Сначала пробуем из JSON-LD Recipe
        recipe_data = self._get_json_ld_recipe()
        if recipe_data and 'keywords' in recipe_data:
            keywords = recipe_data['keywords']
            if isinstance(keywords, str):
                # Проверяем разделитель - может быть запятая или что-то еще
                if ',' in keywords:
                    tags_list = [tag.strip() for tag in keywords.split(',') if tag.strip()]
                else:
                    tags_list = [keywords.strip()] if keywords.strip() else []
            elif isinstance(keywords, list):
                tags_list = [str(tag).strip() for tag in keywords if tag]
        
        # Если не нашли в Recipe, ищем в Article JSON-LD
        if not tags_list:
            article_data = self._get_json_ld_article()
            if article_data and 'keywords' in article_data:
                ...
                if isinstance(keywords, str):
                    ...
                    tags_list = [str(tag).strip().lower() for tag in keywords if tag]
...
def main():
    import os
    # Обрабатываем папку preprocessed/thequeenofdelicious_com
    preprocessed_dir = os.path.join("preprocessed", "thequeenofdelicious_com")
    if os.path.exists(preprocessed_dir) and os.path.isdir(preprocessed_dir):
        process_directory(TheQueenOfDeliciousExtractor, str(preprocessed_dir))
        return
